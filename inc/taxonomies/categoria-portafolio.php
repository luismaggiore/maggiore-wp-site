<?php
if (!defined('ABSPATH')) exit;

function mg_register_tax_categoria_portafolio() {

    $labels = [
        'name'              => __('Categorías de Portafolio', 'maggiore'),
        'singular_name'     => __('Categoría', 'maggiore'),
        'search_items'      => __('Buscar Categorías', 'maggiore'),
        'all_items'         => __('Todas las categorías', 'maggiore'),
        'parent_item'       => __('Categoría superior', 'maggiore'),
        'parent_item_colon' => __('Categoría superior:', 'maggiore'),
        'edit_item'         => __('Editar categoría', 'maggiore'),
        'update_item'       => __('Actualizar categoría', 'maggiore'),
        'add_new_item'      => __('Añadir nueva categoría', 'maggiore'),
        'new_item_name'     => __('Nombre de nueva categoría', 'maggiore'),
        'menu_name'         => __('Categorías', 'maggiore'),
    ];

    $args = [
        'labels'            => $labels,
        'hierarchical'      => true,
        'public'            => true,
        'show_ui'           => true,
        'show_in_rest'      => true,
        'show_admin_column' => true,

        // ✅ Opción A: cambiar base para evitar conflicto con /portafolio/...
        // Términos en: /portafolio/categoria/diseno
        'rewrite' => [
            'slug'         => 'portafolio/categoria',
            'with_front'   => false,
            'hierarchical' => true,
        ],
    ];

    register_taxonomy('mg_categoria_portafolio', ['mg_portafolio'], $args);
}

// registrar después del CPT
add_action('init', 'mg_register_tax_categoria_portafolio', 6);
